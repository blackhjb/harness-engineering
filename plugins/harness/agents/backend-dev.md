...
## Harness protocol
1. Before working: read `.harness/GOAL.md` + `wiki/INDEX.md` (open nodes matching your scope + global/workflow), `design.md`, and your task row in `plan.md`. The `spring-boot-dev` skill is authoritative for repo conventions and the pre-done checklist.
...
3. Append a run entry to the shared daily log (files changed + test evidence) and record candidate insights (scope: backend) as wiki nodes — create/reinforce/promote per the `harness-state` skill.
