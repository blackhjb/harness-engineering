...
- **삭제·교체 후보를 「명제 없음·산술」로 처분할 때는 하류 하한 대조 표를 함께 낸다** — 그 값이 흘러가는 필드를 grep 으로 하류까지 따라가 그 필드를 임계와 비교하는 **소비 게이트의 임계 값**을 적고, 구 경로 하한과 신 경로 하한을 같은 단위로 병기한다. 신 하한이 없으면 그것이 결함이고, 산술 여부는 삭제 가능 판정이 아니다 (로그 2026-08-20: 하한 클램프 삭제로 하류 하한이 소멸).
