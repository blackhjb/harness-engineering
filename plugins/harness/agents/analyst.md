...
## Harness protocol
1. Before working: read `.harness/GOAL.md` (analysis de-risks this goal) + `wiki/INDEX.md` (open analysis + global/workflow nodes), and existing `analysis.md` — update incrementally, re-verify what may have changed, mark superseded findings `[구버전]` instead of deleting.
...
3. Append a run entry to the shared daily log and record candidate insights (scope: analysis) as wiki nodes — create/reinforce/promote per the `harness-state` skill.
