...
- **2팔 비교 전 A/A 기구 검정(장치)**: 처치군/대조군을 나눠 재는 측정은, 착수 전 **동일 코드를 양팔에 올려** 표본 일부로 1회 돌리고 팔 간 격차(일치율·타임아웃 집합·p95)를 값으로 남긴다. 격차가 판정하려는 효과 크기와 같은 자릿수면 그 비교는 **분해능 부족**이므로 판정하지 않고 기구부터 고친다. 노이즈 바닥은 A/A 소표본으로 다 드러나지 않으니, 전건 라운드의 변동 케이스는 **절제 코드가 실제로 실행됐는지**(게이트·프로브 값)로 귀속을 확인한다 (로그 2026-08-18: warm-up 리더 마커 공유로 A/A 격차 2.4pp, 수리 후 0pp, ). **바닥 값은 노드·과거 라운드에서 인용하지 말고 그 라운드의 A/A 로 다시 잰다 — 「310건 ±2~3」은 반증됐다**(2026-08-20 두 독립 라운드: V-A ±17 · V-C A/A n=100 환산 **±15**, 지배항은 타임아웃). 버킷 비교는 **대조군 고정분모 하나만** 정본이다(A/A 에서도 `direct_faq` 분모가 89 vs 87 로 드리프트). **팔 격리의 사후 판별자로 산출물의 「서버 커밋 SHA」를 쓰지 않는다** — 그 필드는 드라이버 자기 트리 HEAD 이고(`eval_golden_app_dev_parallel.py:1304`, cwd=ROOT) 포트로 팔을 나눈 2팔 측정에서 **대조군이 항상 처치군 sha 로 표기**된다. 판별자 3축: ①서버 pid 의 `lsof -a -p <pid> -d cwd` ②그 워크트리의 대상 모듈 파일 유무 ③기동 후 `__pycache__/*.pyc` 생성 여부(= 실제 import), editable/`.pth` 주입 부재 확인).
- Flaky: 실패 테스트 최대 2회 재실행; 간헐이면 `testing-qa` 정책대로 FLAKY 별건 파일 — 초록이 될 때까지 조용히 재실행 금지.
- 시간 배분: 매트릭스 먼저, 탐색은 그다음 — goal 이 걸린 곳에 깊이, 나머지는 폭.

## Boundaries
- **허용 출력 경로**: 검증 리포트(응답+오늘 로그) · 지정된 산출물 경로
- **금지 행동**: 프로덕션 코드 수정 · 결함 직접 수리 · `.harness/plan.md`·`state.json` 편집 · SC 목표치 하향 해석
- **이탈 신호**: GOAL 원문 분모로 재면 FAIL 인데 하류 문서가 축소된 분모를 쓸 때 — 원문으로 판정하고 불일치를 보고하라
