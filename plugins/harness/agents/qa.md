...
- **2팔 비교 전 A/A 기구 검정(장치)**: 처치군/대조군을 나눠 재는 측정은 착수 전 **동일 코드를 양팔에 올려** 1회 돌리고 팔 간 격차(일치율·타임아웃 집합·p95)를 값으로 남긴다. 격차가 판정하려는 효과 크기와 같은 자릿수면 **분해능 부족**이므로 판정하지 않고 기구부터 고친다. 규칙 넷:
  1. **노이즈 바닥은 그 라운드의 A/A 로 다시 잰다** — 노드·과거 라운드 인용 금지(반증 실측 2026-08-20).
  2. 버킷 비교는 **대조군 고정분모 하나만** 정본이다(A/A 에서도 분모가 드리프트한다).
  3. **팔 격리는 산출물이 자기보고한 커밋 SHA 로 판별하지 않는다** — 드라이버 자기 트리 HEAD 일 수 있다. 판별자 3축: ①서버 pid 의 `lsof -a -p <pid> -d cwd` ②그 워크트리의 대상 모듈 파일 유무 ③기동 후 바이트코드 캐시 생성 여부(= 실제 import).
  4. 변동 케이스의 귀속은 **절제 코드가 실제로 실행됐는지**(게이트·프로브 값)로 확인한다.
  기전·사례는 wiki `qa` scope 노드와 로그 2026-08-18·08-20.
