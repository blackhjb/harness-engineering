...
2. **Extract cost disproportion as a first-class signal, independent of failures.** 읽는 순서가 정해져 있다 — **산문 로그보다 원장이 정본이다**:

   | 순서 | 읽는 것 | 뽑는 것 |
   |---|---|---|
   | 1 | `.harness/measurements.jsonl` | `refutes` 를 가진 레코드 = **M1 미달 지점** · `rework:true` 디스패치 = **M2 미달 지점**. 이 두 집합이 이번 회고의 **1순위 패턴 후보**다 |
   | 2 | 로그 result 항목이 인용한 원장 id | `grep -c` 로 실재 대조 → **미실재 건수를 보고서에 값으로** 기재. 산문 인용은 append 를 보증하지 않는다(실측 2026-08-20: 표본 10건 전건 미실재, M2 분모 결손) |
   | 3 | 각 goal 의 결산 줄 | diff shortstat · commit count · dispatch count · subagent tokens |

   A goal whose orchestration cost is grossly out of proportion to its diff (e.g., full analyze→plan→design→build ceremony spent on a mechanical batch) is a pattern **even when every task succeeded** — its causal mechanism is usually a routing or scoping rule, and the fix is a bounded proposal against the triage thresholds in the goal command or a `cost`-scope wiki node. A zero-failure retro can still carry a cost pattern (로그 참조).
...
- **「lowest-value」는 판단이 아니라 계수다 — 자동 은퇴(값 트리거).** 은퇴 후보 모집단 = 아래 **세 조건을 모두** 만족하는 active 노드.

  | 축 | 조건 | 이 축이 없으면 |
  |---|---|---|
  | 최근성 | 최근 창 인용 **0** | — |
  | 가치 | 누적 인용 **≤ active 중앙값** | 최근성만 보면 「과거에 많이 쓰였고 지금 잠잠한」 노드를 버린다 (2026-08-28: 최근 창 0 인 2건 중 하나가 **누적 14회**) |
  | 나이 | 등재 후 **2 iteration 이상** 경과 | 신규 노드가 쓸려 나간다 — 인용 0 은 아직 안 쓰인 것이지 불필요한 것이 아니다 |

  가치 축만으로도 안 된다: 성숙한 위키는 전건 인용 ≥1 이라 모집단이 **구조적으로 공집합**이 된다(같은 날 active 40 중 누적 0 = **0건**, 승격 자격 4건이 자리를 못 얻었다). 계수 명령:

  ```
  python3 -c "import json,glob,pathlib;from collections import Counter;rows=[json.loads(l) for f in ['.harness/measurements.jsonl','.harness/measurements.archive.jsonl'] if pathlib.Path(f).exists() for l in open(f) if l.strip()];import statistics;w=Counter(n for r in rows[-400:] if r.get('t')=='d' for n in (r.get('nodes') or []));c=Counter(n for r in rows if r.get('t')=='d' for n in (r.get('nodes') or []));print([p.stem for p in pathlib.Path('.harness/wiki').glob('*.md') if p.name!='INDEX.md' and w.get(p.stem,0)==0 and c.get(p.stem,0)<=statistics.median([c.get(q.stem,0) for q in pathlib.Path('.harness/wiki').glob('*.md') if q.name!='INDEX.md'])])"
  ```

  **승격 자격 노드(≥2 독립 근거일)가 상한으로 2회 보류되면 이 모집단에서 인용 최저 1건을 은퇴시켜 자리를 낸다 — 보류 3회차는 규칙 위반이다.** 모집단이 비면(=전건 인용 ≥1) **상한을 1 넘겨 승격하고 그 사실과 두 계수를 보고서에 값으로 적는다** — 그때는 상한 자체가 재검토 대상이지 새 교훈이 버려질 일이 아니다. (실측 2026-08-28: 기준 부재로 `analysis--indirection-defeats-entrypoint-sealing` 이 **3회 연속 보류**. 동시에 retired 75건 중 **36건이 인용 기록 보유**(최대 12회) — 은퇴가 가치가 아니라 대기열 순서로 결정돼 왔다는 반증이다.)
...
- **Ledger rotation (same turn)** — 동시 append 는 정의상 **꼬리**에 붙고 절단 범위(선두 N행)에는 들어오지 않는다. 그래서 **가드는 파일 전체가 아니라 절단 범위에만 건다.**

  | # | 하는 일 | 값·판정 |
  |---|---|---|
  | 1 | 절단 경계 `N` 을 정한다 | 이 회고가 채굴한 마지막 레코드까지. **열린 goal 의 레코드는 제외**(`state.json` phase 가 done 이 아닌 goal) |
  | 2 | `head -N` 의 해시를 잰다 | — |
  | 3 | archive 직전 **다시** 잰다 | **두 해시가 같을 때만** 진행. 다르면 「회전 보류: 절단 범위 변동」 + 잔여 행수 |
  | 4 | `head -N` → `.harness/measurements.archive.jsonl` 에 append | — |
  | 5 | 활성 파일을 **그 순간 다시 읽은 전체**에서 `tail -n +N+1` 로 재작성 | 꼬리에 새로 붙은 행이 보존된다 |
  | 6 | 활성 파일 **잔여 행수**를 보고서 실행 요약에 기재 | 값이다 |

  **전체 행 수 변동은 보류 사유가 아니라 보고 값이다** (2026-08-27 iter51: 136→137 로 회전을 보류했는데 절단 대상 선두 112행은 무변동이었다). 승계 키는 `<명령>@<sha7>` 로 sha 결박이라 폐쇄 goal 레코드는 히트하지 않는다(실측 2026-08-20: 중복 key 5/347).
