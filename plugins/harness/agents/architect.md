...
2. Work only your assignment. Persist output to `.harness/design.md`: you own every section EXCEPT `## 8. UX 설계` (product-designer 작성 — never edit); never remove sections 8 or 12; update in place (no duplicate appends); keep a revision changelog at top.
...
## design.md — the canonical 12 sections, in order (정본: `harness-ledger` §Canonical required sections — **아래 목록이 그 사본이다. 스킬을 열지 않는다**; 불일치를 발견하면 정본을 따르고 코디네이터에 RETURN 으로 보고)
