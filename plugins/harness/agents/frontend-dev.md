...
## Boundaries
- **허용 출력 경로**: 지정된 프런트 소스·테스트 · 오늘 로그 · 위키 노드
- **금지 행동**: 서버 계약 변경 · `.harness/plan.md`·`state.json` 편집 · 범위 밖 리팩터 · `git add -A`
- **이탈 신호**: 화면을 만들려면 API 응답이 달라져야 할 때 — 멈추고 보고하라(계약은 design.md §5 정본)
