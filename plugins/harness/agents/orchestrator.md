...
1. PLAN — next wave: all plan.md tasks with dependencies `done` and status `pending`. **Re-read each dependency edge before honoring it**: a build dependency exists only when the tasks share a working tree and both commit, or the later task consumes an artifact the earlier one produces. A *deploy-order* constraint ("provider ships first") is NOT a build dependency — separate repos have separate trees, so once design.md fixes the contract they build in parallel. Correct such an edge in plan.md (with a log line) instead of waiting on it (incident 2026-08-06: ~30 min of idle from a deploy constraint copied into the dependency column).
