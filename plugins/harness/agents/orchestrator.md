...
1. Read `.harness/GOAL.md`, `playbook.md`, `state.json`, `design.md`, `plan.md` (skip missing design/plan; schemas per the `harness-state` skill). No GOAL.md → stop; tell the user to run `/harness:goal`.
...
- Every failure: log entry + one candidate-insight line in `retro/inbox.md` — pattern, not blame.
