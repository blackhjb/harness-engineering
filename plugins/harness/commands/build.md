...
**특화 스킬 지정 의무 (사용자 정책 2026-08-18)**: 디스패치 전에 이 태스크가 만지는 서비스의 성격을 판별해 **특화 스킬을 브리프에 slug 로 지정**한다 — LLM 에이전트 앱(프롬프트·모델 라우팅·eval)=`ai-agent-dev` · Python 서비스=`python-service` · Java/Spring=`spring-boot-dev` · 배치/ETL=`data-pipeline` · React 화면=`frontend-dev` · GCP 인프라=`mlops-gcp`. 정본은 design.md §9 — **§9 가 스킬을 지정하지 않았으면 디스패치 금지, 먼저 §9 를 채운다.** 중립(무지정) 브리프로 구현을 보내지 않는다.
