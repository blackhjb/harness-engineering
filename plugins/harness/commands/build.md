...
**모델 티어링 (사용자 정책 2026-08-18)**: 에이전트 frontmatter 가 기본값(설계·분석=fable, 빌드·검증=opus)이다. **골든 측정·스크립트 실행·검증된 지도 위 기계적 삭제/치환 등 판단 0 디스패치는 `model: sonnet` 오버라이드** — 판단이 남아 있는 디스패치에는 내리지 않는다.
