...
3. Each harness edit proposal as a diff with 증거 / 기대 효과 / 회귀 리스크, **위험도 분류 동반**. **저위험**(additive 문구·체크리스트·기록 의무 — 게이트·권한·검증 엄격도를 바꾸지 않음)은 자동 적용 후 사후 통보만 한다; **고위험**(승인 게이트·권한·검증 엄격도·안전 규칙 변경)만 사용자가 건별 승인한다 (사용자 결정 2026-08-14 — "사람이 볼 필요 있나"). Apply approved/auto-applied diffs to the named agent/command files; record each decision (auto-applied/approved/rejected + reason) in the retro report.
