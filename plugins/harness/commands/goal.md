...
## 1-B. 시스템 유형 판별 (스킬 선택의 정본 — 생략 금지)
스캐폴드 전에 **이 goal 이 만지는 시스템이 무엇인지 한 줄로 판별해 GOAL.md 승인 섹션에 기록**한다: `LLM 에이전트 앱` / `Python 서비스` / `Java·Spring 백오피스` / `배치·ETL` / `React 화면` / `GCP 인프라` (복합이면 표면별 병기). 이 판별이 design §9 의 특화 스킬 지정과 analyze·build 전 디스패치 브리프의 스킬 slug 를 정하는 **유일한 정본**이다 — 판별 없이 스킬을 고르는 것은 감이고, 중립 브리프는 금지다 (사용자 정책 2026-08-18: "어떤 서비스인지 판단 후 스킬을 선택해야").
