...
- **예상 산출**: 이 goal 이 끝나면 `src/` 프로덕션에서 **몇 줄·몇 분기**가 없어지는가(또는 어떤 사용자 가치가 배포되는가). 「측정 축을 세운다」·「계측을 붙인다」·「인벤토리를 만든다」는 **산출이 아니다** — 그것들을 소비해 무엇이 없어지는지까지 적어야 산출이다. **값은 실측 인용만**: `complexity_scan.py`·`git grep` 실행값 또는 원장 키를 병기하고, 상위 후보 표(도메인·블록별 상금 top-3)를 함께 적어 선택 표적이 최대가 아니면 사유 1줄 — 실측 없는 추정치는 기재 불가 — 표적 미확정 신호다(2026-08-20 미실측 「SCORE 204」 vs 실측 최대 48분기).
- **예상 디스패치 수**: analyze·design·build 웨이브·verify·retro 합계.
- **감축분은 행으로 지목한다(장치)**: 목표치가 「X→Y」·「−N」 형태면 그 SC 의 **판정 명령을 기준선에서 그 자리에 1회 실행**하고, 출력에서 없어질 항목을 `file:line` **N행 목록**으로 GOAL 승인 섹션에 적는다. **지목 행 수 < N 이면 목표치는 지목 행 수다**(내림, 사유 1줄). 기준선만 재고 감축분을 유추한 목표치는 기재 불가 — iter30~33 **4회 연속** 반증된 값은 전부 기준선이 아니라 **감축분**이었고 기전은 「인용값의 축·명령이 목표치와 다름」이다(iter33: 231줄을 재고 「≤35」 — 231행 중 **96행이 시그니처·반환형 계약**이라 같은 GOAL 이 금지 · 분기 16을 재고 자 `SCORE −16` — 그 16행은 **SCORE 축에 없었다**). 목표치의 축은 판정 명령의 축과 **같은 이름**이어야 한다 — 다르면 그 목표치는 미측정이다.
- **증거 가능성 — 장치**: 삭제·교체형 goal 은 **표적 코드가 검증 코퍼스에서 실제로 실행되는 횟수**를 스캐폴드 전에 값으로 적는다 — 진입 마커 1회 계수(`grep -c '<진입 로그 문자열>' <대조군 팔 로그>`, **생산자 로그 레벨을 함께 확인**) 또는 진입 게이트 계수. **계수 대상은 그 게이트의 진입 술어 전건의 논리곱이다**: 표적 술어를 소스에서 그대로 옮겨 적고(`<술어 원문 + file:line>`) 절이 N개면 **N개 절을 모두 만족한 케이스 수**를 하나의 값으로 낸다 — 절 하나의 발생 수(상류 조건·라우트 분포·타임아웃 건수)는 **그 값의 대리가 아니다**. 절별 계수를 낼 수 없으면 진입 마커 로그를 먼저 심고 계수한다. **이 값이 0 이면 그 코퍼스로 어떤 SC 도 성립하지 않는다** — 「진입 픽스처 조달」을 SC-0 으로 등재하고 진입 계수 >0 실증을 build 착수 전 인수조건에 넣는다. **유추 상한(라우트 분포·발동률)도, 절 하나의 발생 수도 이 값의 대리가 아니다** — 실례: iter30 유추 ≤65 vs 직접 계수 **0**, iter31 논리곱 2절 중 1절만 계수(후자 0건) → 둘 다 발동 0·원복·순산출 0. 진입 마커가 없는 코드(로그 0줄 모듈)는 **AST 도달성 + 호출부 grep** 을 대체 증거로 허용한다.

- **표적 종류 — 장치**: 삭제 후보가 「거부·롤백·차단·검증」 계열(**가드**)이면 **코퍼스 진입 계수·ablation 델타로는 삭제 판정을 낼 수 없다**. 가드는 「죽음」과 「방어 대상 사건 미발생」이 같은 관측(발동 0)을 내기 때문이다. 이 경우 **구/신 구현의 적대적 차등 실행**을 SC-0 으로 등재한다 — 크래프트 입력은 그 가드의 **거부 사유 라벨에서 역산**하고(사유가 문자열이면 그 조건을 만족하는 쌍을 만든다), 믹스인 메서드는 **실제 host 객체를 `self` 로** 넘긴다. 판정이 한 쌍이라도 갈리면 **표적에서 제외**한다. 상위 후보 표에는 상금(분기 수) 옆에 **표적 종류(가드 / 배선 / 계측)** 열을 적는다 — 종류 미기재는 표적 미확정과 같다. 실례 2026-08-24 iter36: typo 3블록이 **390건 발동 0 + ablation +3**(A/A 하한 밖)을 통과했으나 차등 11쌍 중 **6쌍 반전** → **−168줄 전량 원복**.

**게이트**: 삭제형·구조형 goal 인데 **예상 산출(분기 델타) < 예상 디스패치 수**면 그 goal 을 세우지 않는다. 표적을 바꾸거나(상금표 상위에서 더 큰 블록 — **SCORE 축을 인용하면 식별자 덤프로 오탐 분리 선행**) quick 경로로 내린다. 예상치를 못 대면 그것도 신호다 — 표적이 확정되지 않은 것이므로 먼저 grep 으로 규모를 재라.
