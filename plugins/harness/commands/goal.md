...
**SC 표 확정 체크리스트 — 행마다 6축 전부.** 범위 팽창과 vacuous SC 는 실행이 아니라 이 표를 쓰는 순간 태어난다 (로그 2026-08-07):
...
6. **기준선 실존** — SC 가 기준선 수치·측정 도구를 인용하면, 그 수치의 원장 키(`<명령>@<sha7>`) 또는 provenance 산출물 경로를 같은 행에 병기하고 지정 도구는 `git ls-files` 로 지금 실존 확인한다. 산문 로그 1줄만이 출처인 수치는 기준선이 아니라 `[미검증]` 참고값이다 (로그 2026-08-14 SC-4 — 재현 불가 기준선이 verify FAIL 로 되돌아왔다).
