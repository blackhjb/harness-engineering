...
7. **소비자 결박** — SC 의 산출물이 계측·게이트·리포트처럼 **측정 수단**이면, 그 측정을 소비해 코드를 처분하는 SC 를 **같은 goal 안에** 함께 적는다(1~6축은 계측 부착에 전부 통과하므로 이 축이 없으면 걸러지지 않는다). 소비가 다음 goal 로 밀리면 남는 것은 부착 세금(계측 사이트 + lint allowlist 이름)뿐이고, 소비가 무산되면 **원복이 기본값**이다. **예외**: 수집 창이 이 goal 안에 닫히지 않는 계측(실트래픽 대기 등)은 소비 SC 를 미룰 수 있으나 ①미루는 사유 ②소비 goal 의 착수 조건 ③무산 시 원복 책임을 GOAL.md 승인 섹션에 **명시**한다 — 기록 없는 유예가 이 축의 위반이다 (로그 2026-08-18 iter27: 계측 +25 사이트·삭제 0, 이후 회수 권고 15/52 사이트).
...
- **예상 산출**: 이 goal 이 끝나면 `src/` 프로덕션에서 **몇 줄·몇 분기**가 없어지는가(또는 어떤 사용자 가치가 배포되는가). 「측정 축을 세운다」·「계측을 붙인다」·「인벤토리를 만든다」는 **산출이 아니다** — 그것들을 소비해 무엇이 없어지는지까지 적어야 산출이다.
