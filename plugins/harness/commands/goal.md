...
- **증거 가능성 — 장치**: 삭제·교체형 goal 은 **표적 코드가 검증 코퍼스에서 실제로 실행되는 횟수**를 스캐폴드 전에 값으로 적는다 — 진입 마커 1회 계수(`grep -c '<진입 로그 문자열>' <대조군 팔 로그>`, **생산자 로그 레벨을 함께 확인**) 또는 진입 게이트 계수. **이 값이 0 이면 그 코퍼스로는 어떤 SC 도 성립하지 않는다**: goal 을 세우려면 「진입 픽스처 조달」을 SC-0 으로 등재하고 그 픽스처의 진입 계수 >0 실증을 build 착수 이전 인수조건에 넣는다. **라우트 분포·발동률에서 낸 유추 상한은 이 값을 대체하지 못한다**(2026-08-20 iter30: 유추 ≤65 vs 직접 계수 **0** → SC-4·SC-6·SC-3 동시 판별력 0, 15분기 삭제가 무증거 삭제로 남았다). 진입 마커가 없는 코드(로그 0줄 모듈)는 **AST 도달성 + 호출부 grep** 을 대체 증거로 허용한다.
