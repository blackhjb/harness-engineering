...
Classify the goal's size BEFORE scaffolding. **The change surface MUST be grep-measured first — routing on document reasoning alone is forbidden**: locate the files that actually change (the constant, the route, the component) and count them, then write that count into GOAL.md 승인 섹션 alongside the route. A route chosen without a measured surface is the single most expensive mistake this triage can make (incident 2026-08-06: a goal reasoned to be a cross-repo contract change turned out to be one additive boolean field across 5 files; the full loop cost ~55 min of documents and wave management that quick would not have spent). Full-loop ceremony must be proportional to the change, never to the goal's importance — a 15-line mechanical batch does not earn a PRD (incident: ~300 changed lines consumed ~1.4M subagent tokens on documents and wave management; the value came from verification, not ceremony).
