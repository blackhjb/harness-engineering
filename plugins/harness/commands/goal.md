...
- **증거 가능성 — 장치**: 삭제·교체형 goal 은 **표적 코드가 검증 코퍼스에서 실제로 실행되는 횟수**를 스캐폴드 전에 값으로 적는다 — 진입 마커 1회 계수(`grep -c '<진입 로그 문자열>' <대조군 팔 로그>`, **생산자 로그 레벨을 함께 확인**) 또는 진입 게이트 계수. **계수 대상은 그 게이트의 진입 술어 전건의 논리곱이다**: 표적 술어를 소스에서 그대로 옮겨 적고(`<술어 원문 + file:line>`) 절이 N개면 **N개 절을 모두 만족한 케이스 수**를 하나의 값으로 낸다 — 절 하나의 발생 수(상류 조건·라우트 분포·타임아웃 건수)는 **그 값의 대리가 아니다**. 절별 계수를 낼 수 없으면 진입 마커 로그를 먼저 심고 계수한다. **이 값이 0 이면 그 코퍼스로 어떤 SC 도 성립하지 않는다** — 「진입 픽스처 조달」을 SC-0 으로 등재하고 진입 계수 >0 실증을 build 착수 전 인수조건에 넣는다. **유추 상한(라우트 분포·발동률)도, 절 하나의 발생 수도 이 값의 대리가 아니다** — 실례 2건: iter30 유추 ≤65 vs 직접 계수 **0**(SC 3건 판별력 0, 15분기 무증거 삭제) · iter31 「타임아웃 **AND** `faqId` 존재」를 전자만 30/310 으로 재 goal 을 세웠으나 후자 **0건** → 발동 0·커밋 원복·순산출 0. 진입 마커가 없는 코드(로그 0줄 모듈)는 **AST 도달성 + 호출부 grep** 을 대체 증거로 허용한다.
