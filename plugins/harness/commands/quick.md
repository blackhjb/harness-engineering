...
**quick 이 표준 루프와 다른 점은 정확히 셋뿐이다** — ①`/harness:plan` Stage 4 **승인 게이트를 거치지 않는다**(기계적 변경이라 승인할 설계가 없다) ②분해가 배치 1건이라 웨이브가 없다 ③verify 커맨드 대신 **코디네이터가 종결한다**. 그 외 전부 — 위키 선독, 디스패치 단가, 검증 엄격도, 장부 형식 — 는 표준 루프와 **같은 규칙, 같은 소유자**다. 여기서 다시 정의하지 않는다. 이 파일에 규칙을 복제하면 한쪽만 고쳐질 때 조용히 갈라진다(실측 2026-08-10: 측정 원장 도입 시 verify·quick 을 따로 고쳐야 했다).
...
2. 작업 — 둘 중 하나:
   ...
   - **dev 에이전트 1명 디스패치** — backend-dev / frontend-dev / ai-agent-dev. 브리프를 쓰기 전에 **`harness-ledger` 스킬 §디스패치 계약의 `Dispatch contract` · `Dispatch economy` · `비동기 루프` 세 절을 열어 그대로 승계한다**. quick 고유 조항은 하나뿐: **plan.md 태스크가 없으므로 이 브리프가 곧 과제이자 인수 조건**이며 "자기 plan.md 태스크만" 규칙은 면제된다.
