...
2. Do the work — one of:
   ...
   - **디스패치 단가 규율** (orchestrator 의 dispatch economy 와 동일): ①이미 확인된 사실은 `file:line` 과 함께 브리프에 싣고 "재검증 말고 여기서 출발" 명시 ②**전제가 깨지면 구현하지 말고 중단·보고**하게 할 것 ③직전 에이전트와 영역이 겹치면 신규 스폰 대신 이어서 쓸 것 ④**기계적·되돌리기 쉬움·판정 없음** 3조건 전건 충족 시에만 하위 모델 티어(리뷰·분석·판정·보안 표면은 하향 금지).
3. **Proportional verification (mandatory, not reducible):**
   ...
   - If the change adds or modifies a guard/lint/test-that-must-catch: a **positive control is mandatory** — inject a known violation → red (exit code) → revert → green, logged with the actual commands and outputs. A guard merged without a red reproduction is not done ("초록불 lint 는 없는 lint 보다 나쁘다"). 대조군은 **사용자 관측 행동을 지키는 가드에만** 붙인다 — 테스트 전용 가드나 수집 하한 단언(`수집 ≥N`)으로 vacuity 가 이미 막힌 가드는 생략하고 그 판단 근거를 로그에 한 줄 남긴다(2026-08-06: 5건 중 3건만 행동을 지켰다). **주입했는데 red 가 0건이면 대조군을 포기하지 말고 픽스처의 판별력을 의심하라** — 그 픽스처에서는 결함이 관측 불가능하다는 뜻이므로, 주입 결함이 드러나는 최소 픽스처로 교체하고 교체 사실을 로그에 남긴다(실측 2026-08-06: 가입 2건 픽스처로는 분할 순서를 뒤집어도 로컬 그룹핑이 첫 등장 순서를 보존해 8건 전건 green — region 교차 3건으로 교체해야 red 가 났다).
