...
3. **Proportional verification (mandatory, not reducible):**
   ...
   - If the change adds or modifies a guard/lint/test-that-must-catch: a **positive control is mandatory** — inject a known violation → red (exit code) → revert → green, logged with commands and outputs. **주입은 1회가 아니라 생산자마다 1회다** — 가드가 지키는 행동의 **생산자를 먼저 열거하고**(값: N개) 각각을 되살려 red N회를 확인한다. 자기가 상상한 한 경로만 주입하면 나머지 경로에 대해 가드는 초록불이다(실측 2026-08-07: 생산자 2개 중 1개만 주입해 통과 선언 → 리뷰어가 나머지 1개로 무탐지 실증). 레시피는 `testing-qa` 스킬. 대조군은 **사용자 관측 행동을 지키는 가드에만** 붙인다 — 수집 하한 단언 등으로 vacuity 가 이미 막힌 가드는 생략하고 판단 근거를 로그에 1줄. **주입했는데 red 0건이면 대조군 포기가 아니라 픽스처의 판별력을 의심**하라.
...
4. If this quick run completes a quick-routed GOAL (state.json goal whose route was set to quick by `/harness:goal`): measure EVERY SC in GOAL.md with its stated command — **측정 전 원장 조회, 새 측정은 원장에 append**(`harness-state` §측정 원장) — append a `verify` log entry with the values **plus the goal 결산** (`git diff --shortstat <base_ref>..HEAD`, 커밋 수, 디스패치 수, **원장에서 계수한 M1·M2·M5**; 원장이 비었으면 「미측정」), set state.json `verify` = {verdict, date} and `phase` = "done" on PASS. This closure is the command/coordinator's job — never the dev agent's.
