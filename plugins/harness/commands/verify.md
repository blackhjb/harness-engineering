...
- `qa` brief: read `.harness/GOAL.md`, `wiki/INDEX.md` (open qa + global/workflow nodes), `plan.md`, `design.md`. Check EVERY success criterion SC-n in GOAL.md and every task's 인수 조건 with an actual command, an inspection, or an audit of logged evidence per the scope rule above — **none by assumption**. RETURN a table: 기준 / 검증 방법(실행한 명령) / 결과 / 증거, plus the task↔artifact map. Append full evidence (command output excerpts) to today's log. 두 가지가 판정 자체를 뒤집는다:

  | 게이트 | 판정 | 값 |
  |---|---|---|
  | **인수조건의 수치 출처** | GOAL SC 표에 없는 수치는 **출처를 값으로 병기**한다. 출처가 GOAL 「범위 제외」 절·배경 통계·상금표면 **그 수치를 인수조건으로 쓰지 않는다** | 그래도 쓰려면 같은 턴에 ①그 집합이 라운드 간 **고정**임을 실측(2라운드 분모 값 병기) ②그 분기의 **판별력**(진입 계수 + `git grep -l … -- tests` 파일 수). 하나라도 미충족이면 **기준 결함(blocks-task, 소유자 = 인수조건 작성자)** 이고 verify 결과가 아니다 |
  | **task↔artifact 사상** | plan.md 의 **모든** task 를 `commit sha` / `log result 항목` / 사유 있는 명시적 `deferred` 중 **정확히 하나**에 사상한다 | 미사상 1건 이상 = **FAIL**. SC 검사는 커밋 없는 task 를 보지 못하므로 「SC 전건 충족」과 「계획된 task 가 실행되지 않음」은 **동시에 참일 수 있다** |

  (2026-08-20 V-C D-1: 「multi 12/12 보존」 → 실측 분모 14/13/16 으로 12 고정 집합 부재, 지목 분기 Golden 진입 0 + 단위 테스트 0파일 → 기준 철회.)
