...
4. WAVE GATE — before the next wave, confirm EVERY task of the finished wave has a log entry: commit-producing tasks via sha, **commit-less tasks (gates, investigations, doc checks) via their log entry — these leave no diff and are the ones that get silently skipped** (로그 참조). `done` only with evidence; "the agent said so" is not evidence. **그 항목이 인용한 원장 id 는 표본이 아니라 전수 대조하고 게이트 줄에 「인용 <n>건 · 미실재 <m>건」을 값으로 적는다** — `for id in <인용 목록>; do grep -c -F "\"$id\"" .harness/measurements.jsonl; done`. 미실재 >0 이면 `done` 전에 집행자에게 정정 항목을 요구한다(실측 2026-08-27: 표본 1/8 확인이 8건 전건 오기를 통과시켰고 두 번째 9건은 아예 못 잡았다).
   **그리고 웨이브를 닫기 전 3계수를 값으로 적는다 — 하나라도 불일치면 닫지 않는다**: ①`grep -c '^## .*dispatch' <오늘 로그>` == `grep -c '^## .*result' <오늘 로그>` ②이 웨이브에서 **내가 직접 집행한 배치 수** == 내가 append 한 `d` 행 수 ③`tokens:"unknown"` 인 위임 `d` 수 == 내가 append 한 `m-*-coord-tokens` 보정 행이 덮는 `d-id` 수. 웨이브 종결이 **커밋 축에만 결박되면** 측정·문서 태스크는 `src/` diff 를 안 남겨 조용히 통과하고, 장부 쓰기 트리거가 위임 왕복에만 붙어 있으면 자기 집행분이 통째로 원장 밖에 남는다(실측 2026-08-31: dispatch **1** vs result **21** · `unknown` **27** vs 보정행 **0** · 코디네이터 자기 집행 `d` **0건** → 다음날 감사에서 장부 결함 7건·미등재 16건 적출). dispatch↔result 가 1:N 인 병합 웨이브는 「불일치 사유 1줄」로 통과시키되 그 줄을 회고가 센다.
